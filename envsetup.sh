...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^MotoMania_") ; then
       MotoMania_BUILD=$(echo -n $1 | sed -e 's/^MotoMania_//g')
    else
       MotoMania_BUILD=
    fi
    export MotoMania_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${MotoMania_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    MotoMania_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls device/MotoMania_System/motomania/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the MotoMania model name
            lunch MotoMania_$target-userdebug
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
#    then
#        # if we can't find a product, try to grab it off the MotoMania github
#        T=$(gettop)
#        pushd $T > /dev/null
